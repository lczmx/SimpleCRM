...
class EasyPermission:
    def __init__(self, settings):
        ...
        self._import_module()
        self.permission_seri = []  # 可序列化的权限列表
    ...
    def check_permission(self, func):
        ...
        def inner(*args, **kwargs):
            ...
            auth_user_id = self.get_user_func(request)  # 获取用户的ID，有id则证明已经登录了，该函数可以自定义，动态导入
    ...
    def format_user_permission_data(self, one_perm):
        ...
        self.permission_seri.append(temp)
    ...
    def _import_module(self):
        ...
        *get_user_path, get_user_func_name = self.settings.get("RETURN_USER_ID").split(".")  # 获取返回id的函数，并导入
    ...
    def gen_permission_list(self, auth_user_id, serial=False):
        # 生成权限列表
        # serial为True返回的是一个[{},{},{}]数据
        # 主要用于返回外界，便于序列化
    ...
    def handle(self, request, auth_user_id, *args, **kwargs):
        ...
        if not self.permission_list:
            self.permission_list = self.gen_permission_list(auth_user_id)
        else:
            self.auth_user_id = auth_user_id
            self.permission_list = []
            permission_list = kwargs.get("permission_list")
            for p in permission_list:
                if not isinstance(type(p), PermissionData):
                    self.permission_list.append(PermissionData(**p))
                else:
                    self.permission_list.append(p)
        for permission in self.permission_list:
            ...
            if self.shunt(request):  # 开始正式验证
                if self.is_create_menus():  # 判断是否生成菜单
                    ...
                    CreateMenu(request, models=self.models, is_create_re_type_url=self.create_re_type_url,
    ...
    def _matching_url(self, request):
        ...
        if self._current_permission.is_re:  # 使用re
            ...
            # url可能“/”结尾，也可能不以这个结尾
            # 假如url_path不以/结尾，则urls.py的路由也不以这个结尾
            if url.endswith("/"):
                if url == url_path or url[:-1] == url_path:
                    return True
            else:
                if url == url_path or url + "/" == url_path:
                    return True
        return False
...
class CreateMenu:
    ...
    def process_result_content(self, content):
        ...
        if isinstance(content, MenuData):  # 菜单时
